...
if (!function_exists('money_to_words')) {
    function money_to_words($money, $locale = 'id_ID', $decimalWordFrom = 'titik', $decimalWordTo = 'koma')
    {
        ...
        $word = str_replace($decimalWordFrom, $decimalWordTo, $formatter->format($money));
        return $money ? $word . " Rupiah" : $word;
    }
}
...
if (!function_exists('screenroutes')) {
    function screenroutes($basePath = '', $makeRoute = true, $param = 'zx', $optionalParam = 'zz')
    {
            ...
            ->each(function ($file) use ($param, $optionalParam, $basePath, &$routes, $makeRoute) {
                ...
                if ($makeRoute) {
                    Route::screen($uri, $component)
                        ->name($name)
                        ->breadcrumbs(function (Trail $trail) use ($uri, $title) {
                            return $trail
...
if (!function_exists('money')) {
    ...
    {
        $setting_decimals = setting(Setting::SETTING_KEY_DECIMAL_PLACES, $decimals);
...
if (!function_exists('round_nominal')) {
    function round_nominal($value, $precision = 0, $mode = PHP_ROUND_HALF_UP)
    {
        return ceil($value / 1000) * 1000;
    }
}

if(!function_exists('select2options')) {
    function select2options($query, $key = 'id', $name = 'name', $default = null)
    {
        $rows = $query->get();

        $options = [];
        if (!empty($default)) {
            $selected = $rows->where($key, $default)
                ->first();
...
if (!function_exists('convert_date')) {
    ...
    function convert_date($date = null)
    {
        ...
        $carbon = $date->tz(config('app.timezone'))->toDateTime();
        return $carbon;
    }
}
